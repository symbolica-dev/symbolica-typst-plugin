...
#let _representation(node, ctx) = {
  ...
  let data = _attachment(ctx, "spenso.representation", _name(value), (1, 2))
  if type(data) != array { return none }
  let class = none
  let palette = none
  let row = none
  if data.len() == 2 {
    class = data.at(0)
    palette = data.at(1)
    row = if _name(value) == "spenso::bis" { "bottom" } else { "top" }
  } else if data.len() == 3 {
    class = data.at(0)
    palette = data.at(1)
    row = data.at(2)
...
#let _row-attachment(base, columns, settings) = {
  ...
  for column in columns {
    if column.row == "bottom" {
      top.push(hide(column.source))
      bottom.push(column.source)
    } else {
      top.push(column.source)
      bottom.push(hide(column.source))
    }
  }
  math.attach(
    (base, hide($zws$)).join(),
